...
export default function RemovePages() {
  ...
  const [pagesToRemoveInput, setPagesToRemoveInput] = useState<string>("1");
  const [selectedPagesToRemove, setSelectedPagesToRemove] = useState<Set<number>>(new Set([1]));
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          setError("This PDF is encrypted with a password. Please unlock it before removing pages.");
          return;
        }
        throw loadErr;
      }
      ...
      setFile(uploadedFile);
      setTotalPages(count);
      const defaultRemove = count > 1 ? new Set([1]) : new Set<number>();
      setSelectedPagesToRemove(defaultRemove);
      setPagesToRemoveInput(count > 1 ? "1" : "");
  ...
  const handleInputChange = (val: string) => {
    ...
    if (totalPages > 0) {
      ...
      setSelectedPagesToRemove(parsedSet);
    }
  };
  ...
  const togglePageSelection = (pageNum: number) => {
    ...
    setSelectedPagesToRemove((prev) => {
      ...
      setPagesToRemoveInput(setToString(next));
      return next;
    });
  };

  const applyPreset = (type: "odd" | "even" | "first" | "last" | "clear") => {
    invalidateBlob();
    ...
    if (type === "odd") {
      ...
      newSet = new Set([totalPages]);
    } else if (type === "clear") {
      newSet = new Set();
    }
    setSelectedPagesToRemove(newSet);
    setPagesToRemoveInput(setToString(newSet));
  };
  ...
  const handleRemovePages = async () => {
    ...
    try {
      ...
      if (selectedPagesToRemove.size === 0) {
        throw new Error("Please select at least one page to delete.");
      }

      if (selectedPagesToRemove.size >= totalPages) {
        throw new Error("Cannot delete all pages from the PDF. At least one page must remain.");
      }

      const keepIndices: number[] = [];
      for (let i = 1; i <= totalPages; i++) {
        if (!selectedPagesToRemove.has(i)) {
      ...
      try {
        ...
        if (
          ...
          setIsPasswordProtected(true);
          throw new Error("This PDF is password-protected and cannot be edited without unlocking it first.");
        }
        throw loadErr;
      }
  ...
  const handleDownload = () => {
    ...
    a.download = `pages_removed_${file.name}`;
  ...
  const reset = () => {
    ...
    setSelectedPagesToRemove(new Set());
    setPageThumbnails({});
    setResultBlobUrl(null);
    setError(null);
    setIsPasswordProtected(false);
  ...
  const remainingPagesCount = Math.max(0, totalPages - selectedPagesToRemove.size);
    ...
    <div className="bg-white rounded-3xl border border-slate-200 p-6 md:p-10 shadow-sm">
      ...
      <div className="text-center max-w-2xl mx-auto mb-8 md:mb-10">
        ...
        <p className="text-sm md:text-base text-slate-500 mt-2 leading-relaxed">
          Select and delete unwanted pages or page ranges from your PDF with visual page thumbnails.
        </p>
      </div>
      ...
      {!file ? (
        ...
        >
          ...
          <p className="text-xs md:text-sm text-slate-400 mt-1">
            Choose any multi-page PDF document to trim pages
          </p>
        </div>
      ) : isPasswordProtected ? (
        ...
        <div className="p-6 md:p-8 rounded-3xl bg-amber-50/80 border border-amber-200 text-amber-900 text-center space-y-5 max-w-xl mx-auto my-2 animate-in fade-in">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs md:text-sm text-amber-800 leading-relaxed max-w-md mx-auto">
              <span className="font-semibold text-slate-900">{file.name}</span> is encrypted. Pages cannot be extracted or removed without removing security restrictions first.
            </p>
          </div>
        ...
        <div className="space-y-6 md:space-y-8">
          ...
          <div className="p-5 md:p-6 rounded-2xl border border-slate-200 bg-slate-50/50 space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <label className="text-xs md:text-sm font-bold text-slate-700 uppercase tracking-wider flex items-center gap-2">
                ...
                Pages to Delete (1 to {totalPages})
              </label>
              <div className="flex items-center gap-2 text-xs md:text-sm">
                <span className="text-red-600 font-bold bg-red-50 border border-red-200 px-2.5 py-1 rounded-lg">
                  Deleting: {selectedPagesToRemove.size}
                </span>
            ...
            {/* Quick helper preset pills */}
            <div className="flex flex-wrap items-center gap-2 pt-1">
              <span className="text-xs text-slate-400 font-medium">Quick select:</span>
              <button
                type="button"
                onClick={() => applyPreset("first")}
                className="text-xs font-semibold px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-slate-700 hover:border-red-400 hover:text-red-600 transition-colors cursor-pointer"
              >
                First Page Only
              </button>
              <button
            ...
            <div className="pt-3">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-bold text-slate-600 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  Click any page thumbnail to delete:
                </span>
                {isLoadingThumbnails && (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3.5 max-h-96 overflow-y-auto p-3.5 bg-white border border-slate-200 rounded-2xl">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => {
                  const isMarked = selectedPagesToRemove.has(pageNum);
                  ...
                  return (
                    <button
                      ...
                      className={`group relative rounded-2xl border-2 p-2 flex flex-col items-center gap-2 transition-all cursor-pointer select-none text-left ${
                        isMarked
                          ? "border-red-500 bg-red-50/70 shadow-xs ring-2 ring-red-200"
                    ...
                    >
                      ...
                      <div className="w-full flex justify-between items-center px-1">
                        ...
                        {isMarked ? (
                          <span className="text-[10px] font-extrabold bg-red-600 text-white px-1.5 py-0.5 rounded shadow-xs">
                            DELETE
                          </span>
                        ) : (
                      ...
                      <div className="relative w-full aspect-[3/4] rounded-xl overflow-hidden bg-white border border-slate-200/80 shadow-xs flex items-center justify-center">
                        {thumb ? (
                          <Image
                            ...
                            className={`object-contain p-1 transition-all duration-200 ${
                              isMarked ? "opacity-30 grayscale filter blur-[0.3px]" : "opacity-100"
                            }`}
                            unoptimized
                          />
                        ) : (
                        ...
                        {/* Overlay Banner when marked for deletion */}
                        {isMarked && (
                          <div className="absolute inset-0 bg-red-600/15 backdrop-blur-[0.5px] flex flex-col items-center justify-center gap-1.5 transition-all">
                            <div className="p-2 bg-white rounded-full shadow-md text-red-600">
                              <Trash2 className="w-5 h-5" />
                            </div>
                            <span className="text-[10px] font-black text-red-700 bg-white/95 px-2 py-0.5 rounded-full shadow-xs">
                              Will Be Deleted
                            </span>
                          </div>
                        )}
          ...
          {!resultBlobUrl ? (
            <button
              ...
              disabled={isProcessing || selectedPagesToRemove.size === 0 || selectedPagesToRemove.size >= totalPages}
            ...
            >
              {isProcessing ? (
                ...
                <>
                  ...
                  <span>
                    {selectedPagesToRemove.size === 0
                      ? "Select Pages to Remove"
                      : selectedPagesToRemove.size >= totalPages
                      ? "Cannot Delete All Pages"
                      : `Remove ${selectedPagesToRemove.size} ${
                          selectedPagesToRemove.size === 1 ? "Page" : "Pages"
                        } from PDF`}
                  </span>
                </>
              )}
            ...
            <div className="space-y-4 pt-2">
              <div className="p-5 md:p-6 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center gap-4 text-emerald-900">
                ...
                <div>
                  <h5 className="font-bold text-base md:text-lg">Pages Removed Successfully!</h5>
                  <p className="text-xs md:text-sm text-emerald-700 mt-0.5">
                    Removed {selectedPagesToRemove.size} {selectedPagesToRemove.size === 1 ? "page" : "pages"}. Your new document contains {remainingPagesCount} {remainingPagesCount === 1 ? "page" : "pages"}.
                  </p>
                </div>
              </div>
