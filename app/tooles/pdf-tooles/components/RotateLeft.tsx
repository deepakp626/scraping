...
export default function RotateLeft() {
  ...
  const handleRotateLeft = async () => {
    ...
    try {
      ...
      // Counter-clockwise calculation: subtract rotationAngle from current rotation
  ...
  const handleDownload = () => {
    ...
    a.download = `rotated_left_${rotationAngle}deg_${file.name}`;
  ...
  const rotationOptions = [
    {
      angle: 90 as const,
      label: "90° Left",
      subtitle: "Turn counter-clockwise",
      orientation: "Landscape (90° Left)",
      icon: RotateCcw,
    },
    {
      angle: 180 as const,
    ...
    },
    {
      angle: 270 as const,
      label: "270° Left",
      subtitle: "Three quarter turn left",
      orientation: "Landscape (270° Left)",
      icon: RotateCw,
    },
  ];
    ...
    <div className="bg-white rounded-3xl border border-slate-200 p-6 md:p-10 shadow-sm">
      ...
      <div className="text-center max-w-2xl mx-auto mb-8 md:mb-10">
        <h2 className="text-2xl md:text-3xl font-extrabold text-slate-800 tracking-tight">
          Rotate PDF Left (Counter-Clockwise)
        </h2>
        <p className="text-sm md:text-base text-slate-500 mt-2 leading-relaxed">
          Turn your PDF pages 90°, 180°, or 270° counter-clockwise securely in your browser with instant download.
        </p>
      </div>
      ...
      {!file ? (
        ...
        >
          ...
          <p className="text-xs md:text-sm text-slate-400 mt-1">
            Rotate entire document or specific page numbers • Instant processing
          </p>
        </div>
      ) : isPasswordProtected ? (
            ...
            <div className="border border-slate-200 rounded-2xl p-5 md:p-6 bg-slate-50/50 flex flex-col justify-between space-y-4">
              <div>
                ...
                <p className="text-xs md:text-sm text-slate-500">
                  Target orientation after applying left rotation
                </p>
              </div>
              ...
              <div className="py-8 flex flex-col items-center justify-center">
                ...
                <div className="mt-6 text-center">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-white border border-slate-200 rounded-full text-xs md:text-sm font-bold text-slate-800 shadow-xs">
                    ...
                    {rotationAngle === 90 && "Landscape (90° Left)"}
                    {rotationAngle === 180 && "Inverted (180° Flip)"}
                    {rotationAngle === 270 && "Landscape (270° Left)"}
                  </span>
                </div>
              </div>
          ...
          {!rotatedBlobUrl ? (
            ...
            >
              {isRotating ? (
                <>
                  ...
                  <span>Rotating PDF Document Left...</span>
                </>
              ) : (
                <>
                  <RotateCcw className="w-5 h-5" />
                  <span>Rotate PDF {rotationAngle}° Left</span>
                </>
              )}
            </button>
          ) : (
            <div className="space-y-4 pt-2">
              <div className="p-5 md:p-6 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center gap-4 text-emerald-900">
                ...
                <div>
                  <h5 className="font-bold text-base md:text-lg">PDF Rotated Left Successfully!</h5>
                  <p className="text-xs md:text-sm text-emerald-700 mt-0.5">
                    Your PDF pages have been rotated {rotationAngle}° counter-clockwise. You can now download the new document.
                  </p>
                </div>
              </div>
